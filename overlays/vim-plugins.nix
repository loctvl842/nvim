nixCatsBuilds: inputs: let
  overlay = self: super: {
    nixCatsBuilds = {
      neovim-session-manager = self.vimUtils.buildVimPlugin {
      ...
      neovim-project =  self.vimUtils.buildVimPlugin {
        name = "neovim-project";
        src = self.fetchFromGitHub {
          owner = "coffebar";
          repo = "neovim-project";
      ...
      icon-picker-nvim = self.vimUtils.buildVimPlugin {
      ...
      mini-indentscope = self.vimUtils.buildVimPlugin {
