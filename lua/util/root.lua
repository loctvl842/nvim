...
function M.detectors.lsp(buf)
  ...
  for _, client in pairs(CoreUtil.lsp.get_clients({ bufnr = buf })) do
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  CoreUtil.info(lines, { title = "CoreUtil Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    CoreUtil.root.info()
  end, { desc = "CoreUtil roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("CoreUtil_root_cache", { clear = true }),
