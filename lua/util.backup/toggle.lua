...
function M.option(option, silent, values)
  ...
  if not silent then
    if vim.opt_local[option]:get() then
      CoreUtil.info("Enabled " .. option, { title = "Option" })
    else
      CoreUtil.warn("Disabled " .. option, { title = "Option" })
    end
  end
end
...
function M.number()
  if vim.opt_local.number:get() or vim.opt_local.relativenumber:get() then
    ...
    CoreUtil.info("Enabled line numbers", { title = "Option" })
  end
end
...
function M.diagnostics()
  ...
  if enabled then
    ...
    CoreUtil.info("Enabled diagnostics", { title = "Diagnostics" })
  else
    vim.diagnostic.disable()
    CoreUtil.warn("Disabled diagnostics", { title = "Diagnostics" })
  end
end
...
function M.maximize(state)
  ...
  vim.api.nvim_create_autocmd("ExitPre", {
    once = true,
    group = vim.api.nvim_create_augroup("CoreUtil_restore_max_exit_pre", { clear = true }),
